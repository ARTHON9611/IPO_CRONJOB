...
// In-process singleflight: concurrent misses on the same key share one
// in-flight fetch instead of stampeding the database. (Per serverless
// instance — N instances collapse to N queries, not N x concurrency.
// A Redis SET NX distributed lock is the future step if that ever matters.)
...
export async function getCached(key, ttlSeconds, fetcher) {
  ...
  if (!p) {
    // NOTE: no await between map check and map set — the entry must be
    // visible synchronously or concurrent misses won't coalesce.
    p = (async () => {
      try {
